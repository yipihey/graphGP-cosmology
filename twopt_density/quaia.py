"""Quaia quasar catalog: real-data loader, selection-function-aware
random generator, and Quaia-shape mock generator.
...
metadata columns); the published random catalog only carries
``(ra, dec, ebv)``.

Two paths to a usable random:

  1. Download the published random FITS file (~150 MB) and assign z's
     from the data n(z) -- ``load_quaia(catalog_path, randoms_path)``.
  2. **No random download needed**: read the small (~400 KB) selection
     function healpix map and sample randoms locally weighted by
     completeness. ``make_random_from_selection_function(sel_map,
     n_random, z_data)``. Equivalent to (1) up to Poisson noise on the
     pixel-level rate.

This module provides::

  load_selection_function(path) -> (sel_map, nside)
  make_random_from_selection_function(sel_map, n_random, z_data, ...)
      -> (ra, dec, z) random sample

  load_quaia(catalog_path, randoms_path=None, selection_path=None,
             n_random_factor=10, ...) -> QuaiaCatalog
      Read the data FITS. If ``randoms_path`` is given, load it. Else
      if ``selection_path`` is given, generate a random locally from
      the selection map.

  make_mock_quaia(n_data, n_random, fid_cosmo, ..., seed=...) -> QuaiaCatalog
      Synthetic Quaia-shape catalogue (galactic mask + bimodal n(z) +
      Gaussian-blob clustering); useful when neither the data nor the
      selection map is available.
"""
...
def load_selection_function(path: str) -> Tuple[np.ndarray, int]:
    """Read the published Quaia healpix selection-function FITS map.

    The Storey-Fisher+ ``selection_function_NSIDE64_G20.{0,5}.fits``
    files store the completeness as a single-column binary table with a
    1024-element-per-row layout that ravels to ``12 * NSIDE^2`` pixels in
    ``healpy`` RING ordering.

    Returns
    -------
    sel : (NPIX,) float64 array, completeness in [0, 1]
    nside : int (e.g. 64)
    """
    ...
    with fits.open(path) as hdul:
        hdr = hdul[1].header
        nside = int(hdr["NSIDE"])
        col = hdr.get("TTYPE1", "T")
        sel = np.asarray(hdul[1].data[col], dtype=np.float64).ravel()
    npix = 12 * nside ** 2
    if sel.size != npix:
        raise ValueError(
            f"selection map has {sel.size} pixels, expected NSIDE={nside} -> {npix}"
        )
    return sel, nside
...
def make_random_from_selection_function(
    ...
    """Selection-aware random catalog generated locally -- no Zenodo random
    download needed.

    Sample healpix pixels weighted by completeness, then draw uniform
    random positions within each chosen pixel, and assign redshifts by
    sampling the data's empirical n(z). The result is statistically
    equivalent to the published random catalog (which is generated by
    the same recipe), up to Poisson noise on the pixel-level rate.

    Parameters
    ----------
    sel_map : (NPIX,) array
        Completeness in [0, 1]. As loaded by ``load_selection_function``.
    n_random : int
        Number of random objects to generate (typically 10 x N_data).
    z_data : array
        Redshifts of the data, used as the empirical n(z) for resampling.
    nside : int, optional
        Healpix NSIDE; inferred from sel_map size if not given.

    Returns
    -------
    ra, dec, z : 1D arrays (ra, dec in degrees, z dimensionless)
    """
    import healpy as hp
    ...
    # 1. pixel choice weighted by completeness
    ...
    # 3. assign redshifts by resampling the data n(z)
    z = _sample_z_from_data(z_data, n_random, rng)
    return ra, dec, z
